...
const PlantCard = ({ val }) => {
    ...
    // Some of these videos don't reliably show YouTube's own in-player
    // fullscreen button on mobile (inconsistent per-video, not something
    // our iframe attributes control), so every card gets its own
    // fullscreen toggle instead of depending on that button. The overlay
    // is rendered through a portal straight into document.body rather
    // than in place: several ancestors on this page (AOS scroll-in
    // animations, e.g. the "container-blue" wrapper) sit on a lingering
    // CSS transform, and a transformed ancestor becomes the containing
    // block for any "position: fixed" descendant instead of the real
    // viewport - which silently breaks a fixed-position overlay left
    // in place. Escaping to body via a portal sidesteps that entirely.
    useEffect(() => {
...
return (
  <div className="plant-card">
    ...
    {isFullscreen && ReactDOM.createPortal(
      ...
      >
        <iframe
          title={`${val.name} Plant Video Fullscreen`}
